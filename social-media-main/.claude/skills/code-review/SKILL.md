...
Data handling: the CLI sends code diffs to the CodeRabbit API for analysis. The diff sent depends on the review mode (`-t all`/`committed`/`uncommitted`, `--base`, `--base-commit`), so before running a review confirm there are no secrets or credentials in the scope that mode will send — not just staged changes (committed and all-changes modes ship that content too). Use the narrowest token scope when authenticating (`coderabbit auth login`).
